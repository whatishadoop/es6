...
/***/ (function(module, exports, __webpack_require__) {
	...
	var copyProperties = function copyProperties(target, source) {
	  ...
	  try {
	    for (var _iterator = Reflect.ownKeys(source)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
	      var key = _step.value;
	      //映射用于object对象拿不到，Reflect.ownKeys获取所有属性
	      if (key !== 'constructor' && key !== 'prototype' && key !== 'name') {
	        //判断是否有构造，原型以及name,选择性拷贝
	...
	var Lottery = function (_mix) {
	  ...
	  function Lottery() {
	    ...
	    _this.cur_play = 'r5';
	    //方法的初始化
	    _this.initPlayList(); //在base.js模块中
	    _this.initNumber(); //在base.js模块中
	    _this.updateState();
	    _this.initEvent();
	    return _this;
	  }
	...
	var Base = function () {
	  ...
	  _createClass(Base, [{
	    ...
	    value: function initPlayList() {
	      //给其它模块共享基本信息
	      this.play_list.set('r2', { //是map结构存放 r2 = obj对象
	        bonus: 6,
	        tip: '从01～11中任选2个或多个号码，所选号码与开奖号码任意两个号码相同，即中奖<em class="red">6</em>元',
	        name: '任二'
	      }).set('r3', { // set级联操作往map里添加键值对象
	        bonus: 19,
	    ...
	    value: function initNumber() {
	      for (var i = 1; i < 12; i++) {
	        this.number.add(('' + i).padStart(2, '0')); //this.number是个set对象，里面元素不重复，padStart让每个字符串保持两个字节长度，不足补0
	      }
	    }
	    ...
	    value: function setOpenCode(code) {
	      ...
	      try {
	        for (var _iterator2 = code.values()[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
	          ...
	          self.open_code.add(item); //set集合号码，11个号码选5个不重复号码
	        }
	      } catch (err) {
	        _didIteratorError2 = true;
	        _iteratorError2 = err;
	    ...
	    value: function toggleCodeActive(e) {
	      var self = this;
	      var $cur = (0, _jquery2.default)(e.currentTarget); //$(e.currentTarget) 获取当前的dom对象转换为jquery对象
	    ...
	    value: function changePlayNav(e) {
	      var self = this;
	      var $cur = (0, _jquery2.default)(e.currentTarget); //获取当前点击的jquery对象
	      $cur.addClass('active').siblings().removeClass('active'); //激活标签添加类名，然后其它节点取消这个激活类
	      self.cur_play = $cur.attr('desc').toLocaleLowerCase();
	      (0, _jquery2.default)('#zx_sm span').html(self.play_list.get(self.cur_play).tip); //获取玩发说明
	      (0, _jquery2.default)('.boll-list .btn-boll').removeClass('btn-boll-active'); //切换玩法，将上次激活号码清空
	      self.getCount(); //重新计算
	    }
	    ...
	    value: function assistHandle(e) {
	      e.preventDefault(); //取消缺省事件监听
	      var self = this;
	      var $cur = (0, _jquery2.default)(e.currentTarget);
	      var index = $cur.index(); //返回当前选中索引
	      //清空之前选项
	    ...
	    value: function addCodeItem(code, type, typeName, count) {
	      //字符串模板使用，替代字符串拼接问题，支持${count>1?'复式':'单式'} 表达式写法
	      var self = this;
	      //反引号定义字符串
	    ...
	    value: function getCount() {
	      ...
	      var tpl = void 0;
	      var c1 = win1 < 0 && win2 < 0 ? Math.abs(win1) : win1; //es6的Number取代了Math功能
	    ...
	    value: function getRandom(num) {
	      var arr = [],
	          index = void 0;
	      var number = Array.from(this.number); //设置随机号码的空间，es6支持的把集合(map,set)转化为数组
	      while (num--) {
	...
	var Timer = function () {
	  ...
	  _createClass(Timer, [{
	    ...
	    value: function countdown(end, update, handle) {
	      ...
	      var self = this; //函数对象本身，目的是为了保证 this 指向保持不变
	      if (now - end > 0) {
	        handle.call(self);
	      } else {
	...
	var Calculate = function () {
	  ...
	  _createClass(Calculate, [{
	    ...
	    value: function computeCount(active, play_name) {
	      var count = 0;
	      var exist = this.play_list.has(play_name); //set集合has判断
	      var arr = new Array(active).fill('0'); //es6指定初始化长度数组并填充数值0
	        ...
	        if (size === arrLen) {
	          ...
	          for (var i = 0; i < arrLen; i++) {
	            ...
	            if (size === 1) {
	              ...
	              newArr.splice(0, i + 1);
	              f(newArr, size - 1, newResult); //地推调用必须起个匿名函数，否则报错
	            }
	          }
	        }
	      })(arr, size, []); //es6数组递归调用方法
	      return allResult;
	    }
	  }]);
	...
	var Interface = function () {
	  ...
	  _createClass(Interface, [{
	    ...
	    value: function getOmit(issue) {
	      var self = this; //将上下文this缓存到一个变量中，此处是Interferce类型实例上下文，若被lottery继承则是lottery类型实例上下文，方便下面的promise函数中，通过闭包方式调用this,避免直接调用this 错误
	      return new Promise(function (resolve, reject) {
	        //箭头函数等价于 function(resolve,reject)
